...
export default async function Preferencias() {
  ...
  return <AdminShell><div className="mx-auto max-w-[900px] px-5 py-8 sm:px-8"><p className="text-[13px] text-black/45">Configuración de tu workspace</p><h2 className="mt-1 text-3xl font-bold tracking-[-.06em]">Preferencias</h2><form action={updateSettings} className="mt-8 grid gap-4">
    ...
    <section className="rounded-2xl border border-black/10 bg-[#e8e7df] p-5 sm:p-7"><div className="flex items-start justify-between gap-4"><div><p className="mono text-[9px] uppercase tracking-[.15em] text-black/40">Valores por defecto</p><h3 className="mt-4 text-[16px] font-bold">Atajos para cargar clientes</h3><p className="mt-2 max-w-[560px] text-[12px] leading-5 text-black/50">Se prellenan al crear un cliente, pero cada plan puede modificarlos.</p></div><span className="grid h-9 w-9 shrink-0 place-items-center rounded-full bg-[#d7f85b] text-[#33420d]"><Icon name="sparkle" /></span></div><div className="mt-6 grid gap-4 sm:grid-cols-2"><Field label="Servicio más común" name="defaultServiceName" required className="sm:col-span-2" defaultValue={settings.defaultServiceName ?? "Servicio mensual"} /><label className="text-[12px] font-semibold">Moneda predeterminada<select name="defaultCurrency" className={input} defaultValue="ARS"><option value="ARS">Pesos argentinos (ARS)</option></select></label><label className="text-[12px] font-semibold">Frecuencia más común<select name="defaultFrequency" className={input} defaultValue={settings.defaultFrequency === "CUSTOM" ? "MONTHLY" : settings.defaultFrequency}><option value="WEEKLY">Semanal</option><option value="BIWEEKLY">Cada 15 días</option><option value="MONTHLY">Mensual</option></select></label><Field label="Importe predeterminado" name="defaultAmount" inputMode="decimal" required defaultValue={String(settings.defaultAmount / 100)} /><Field label="Día de vencimiento" name="defaultDueDay" type="number" min="0" max="31" required defaultValue={String(settings.defaultDueDay)} /></div></section>

...
import { billingPeriods, businessSettings, clients, paymentConnections, servicePlans } from "@/lib/db/schema";
import { formatMoney } from "@/lib/money";
import { formatDate, paymentStatus } from "@/lib/ui";

export default async function Admin() {
  const context = await getCurrentContext();
  const [periods, activeClients, settings, connection, planCatalog] = await Promise.all([
    ...
    db.select({ id: servicePlans.id }).from(servicePlans).where(and(eq(servicePlans.organizationId, context.organizationId), eq(servicePlans.status, "ACTIVE"))).limit(1),
  ]);
  const currentSettings = settings[0];
  const currentConnection = connection[0];
  const currentPlan = planCatalog[0];
  ...
  const setup = { payment: currentConnection?.status === "CONNECTED", client: activeClients.length > 0, plan: Boolean(currentPlan), settings: Boolean(currentSettings) };
  ...
  return <AdminShell><div className="mx-auto max-w-[1100px] px-5 py-8 sm:px-8">
    ...
    {(!setup.payment || !setup.client || !setup.plan) && <section className="mb-5 rounded-2xl border border-[#bfd866] bg-[#e1f39a] p-5"><p className="mono text-[9px] uppercase tracking-[.15em] text-[#4e6218]">Primeros pasos</p><h3 className="mt-3 text-xl font-bold tracking-[-.04em]">Dejá Oasis listo para cobrar</h3><div className="mt-4 grid gap-2 text-[12px] sm:grid-cols-3"><a href="/admin/preferencias" className={`rounded-xl border px-3 py-3 font-semibold ${setup.payment ? "border-black/10 bg-white/30 text-black/45 line-through" : "border-black/15 bg-white/60"}`}>{setup.payment ? "✓ Mercado Pago conectado" : "1. Conectar Mercado Pago →"}</a><a href="/admin/planes/nuevo" className={`rounded-xl border px-3 py-3 font-semibold ${setup.plan ? "border-black/10 bg-white/30 text-black/45 line-through" : "border-black/15 bg-white/60"}`}>{setup.plan ? "✓ Plan creado" : "2. Crear un plan →"}</a><a href="/admin/clientes/nuevo" className={`rounded-xl border px-3 py-3 font-semibold ${setup.client ? "border-black/10 bg-white/30 text-black/45 line-through" : "border-black/15 bg-white/60"}`}>{setup.client ? "✓ Primer cliente creado" : "3. Crear primer cliente →"}</a></div></section>}
